...
df = df.withColumn("_source_file", input_file_name()).withColumn("_ingested_at", current_timestamp()).withColumn("_file_date", regexp_extract(input_file_name(), r"(\d{8})\.csv", 1))
...
(
    ...
    .toTable(f"my_catalog.landing.{source_name}")
)
